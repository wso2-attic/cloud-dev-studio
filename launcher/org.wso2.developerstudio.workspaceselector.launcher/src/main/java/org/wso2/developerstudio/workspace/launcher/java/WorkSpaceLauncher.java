...
*/
		...
		btnOk.addSelectionListener(new SelectionAdapter() {
			public void widgetSelected(SelectionEvent e) {
				try {
					...
					logger.error("Workspace Location saving process has failed ! due to  " + e1.getMessage(),e1);
				}
			}
		});
	...
	private String getWorkSpaceLoc() {
		...
		try {
			...
			logger.error("error in reading the workspace directory from properties due to " + e.getMessage(),e);
		}
	...
	private void setWorkSpaceLoc(String selectedDirectory) {
		try {
			...
			logger.error("error in writing the workspace directory to IDE properties due to " + e1.getMessage(),e1);
		}
	}
